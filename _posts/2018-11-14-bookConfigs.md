...
# 在阅读本文之前
大部分同学并不需要阅读此文，默认的书源已提供了大量的信息。大部分书籍可找到，一本书一般都有20个以上的来源了。

书源并非越多越好，满足你的需求就够了。

若你对默认提供的书源不满意，并且具备数据分析，至少懂点**html**和**xpath**的知识，那你往下阅读，才会有所收益。否则，建议您不要阅读本文。直接使用[自定义来源](/2018/02/23/sourceConfigs/)

先解析下所谓的**自定义来源**与**自定义书源**的区别，前者只是针对一个url。后台针对的是整个应用的配置。
...
支持以：**attr1.attr2.attr3[2].obj** 的方式解析出对应的数据。
...
### 更多响应配置
* 优化编码方式：若出现原始数据全为乱码时，可通过改动此值要修改。一般都是**Utf8** 或是 **GB2312** 之类的
